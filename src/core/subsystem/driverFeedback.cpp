...
void DriverFeedback::initializeDevices() {
  this->devices = {};
  addMotorGroup("RDri", &Robot::Motors::rightSide);
  addMotorGroup("LDri", &Robot::Motors::leftSide);
  addMotorGroup("Lift", &Robot::Motors::lift);
  addMotorGroup("Int", &Robot::Motors::intake);
  addDevice("VPod", new Device(&Robot::Sensors::vertTracker));
...
void DriverFeedback::update() {
  ...
  for (int i = 0; i < msgs.size(); i++) {
    Robot::Subsystems::master->print(i, msgs[i].c_str());
  }
  printf("nnnn\n");

  if (newDisconnect) {
    Robot::Subsystems::master->vibrate(".");
